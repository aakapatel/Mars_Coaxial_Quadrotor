...
Quad.g = 9.81;     % Mars Gravity (m/s^2)

% Quadrotor Physical Parameters
Quad.m = 1.2;      % Quadrotor mass (kg)
Quad.l = .9;     % Distance from the center of mass to the each motor (m)
...
Quad.max_motor_speed = 500; % motors upper limit (rad/s)
...
% PID parameters
Quad.X_KP = 0.35;          % KP value in X position control
Quad.X_KI = 0.25;            % KI value in X position control
Quad.X_KD = -.35;         % KD value in X position control
Quad.X_KI_lim = .3;         % Error to start calculating integral term

Quad.Y_KP = 0.35;          % KP value in Y position control
Quad.Y_KI = 0.25;            % KI value in Y position control
...
Quad.Z_KD = -2.5;  % KD value in altitude control
...
% 
% x = [0 0.5 1 1 1 0.5 0 -0.5 -1 -1 -1 -0.5 0];
% 
% y = [0 0 0 0.5 1 1 1 1 1 0.5 0 0 0];
